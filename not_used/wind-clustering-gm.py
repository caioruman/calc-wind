...
def main():
  ...
  for lat, lon, name in zip(lats, lons, stnames):
    ...
    for month in range(1,13):
      ...
      centroids_n, histo_n, perc_n = gm_probability(df_n)
      ...
      centroids_p, histo_p, perc_p = gm_probability(df_p)
...
def plot_wind(centroids, histo, perc, datai, dataf, name, ptype, month):
  ...
  plt.savefig('Images_GM/{0}_{1}{2}_{3}_{4}_gm.png'.format(name, datai, dataf, ptype, month))
  plt.close()

  return None

def gm_probability(df):
  '''
    For now fixed at 2 clusters

    returns: Array of the centroids, the two histograms and % of each group

    Now using Gaussian Mixture
  '''
  ...
  return gm.means_, [hist_0, hist_1], [df_0.shape[0]*100/df_a.shape[0], df_1.shape[0]*100/df_a.shape[0]]
